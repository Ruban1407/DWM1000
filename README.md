...
After a short known processing delay, the Responder sends a reply packet. The DW1000 records the exact moment it goes out. The response payload carries T2 and T3 so the Initiator can use them.
...
Now the Initiator has all four timestamps. The math removes the time the Responder spent processing and leaves only the time the signal spent traveling through the air.
...
A Time of Flight of 33.4 ns gives roughly 10 metres.
...
The centimeter-level accuracy comes entirely from the DW1000's hardware timestamping. The STM32 could not achieve this on its own.
